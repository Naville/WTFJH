...
extern void init_Frida_hook(){
	 ...
	 for(int i=0;i<_dyld_image_count();i++){
        ...
        if([curName containsString:WTFJHTWEAKNAME]){
            ...
            char * data=getsectdatafromheader(selfHeader,"WTFJH","Frida",&size);

#else
            uint64_t size=0;
            const struct mach_header_64*   selfHeader=(const struct mach_header_64*)_dyld_get_image_header(i);
            char * data=getsectdatafromheader_64(selfHeader,"WTFJH","Frida",&size);
#endif
...
NSString* BundledDYLIBPath=[NSString stringWithFormat:@"%@/frida-gadget.dylib",[NSBundle mainBundle].bundlePath];

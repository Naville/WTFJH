...
extern void init_Reveal2_hook(){
	 ...
	 for(int i=0;i<_dyld_image_count();i++){
        ...
        if([curName containsString:WTFJHTWEAKNAME]){
            ...
            char * data=getsectdatafromheader(selfHeader,"WTFJH","Reveal2",&size);

#else
            uint64_t size=0;
            const struct mach_header_64*   selfHeader=(const struct mach_header_64*)_dyld_get_image_header(i);
            char * data=getsectdatafromheader_64(selfHeader,"WTFJH","Reveal2",&size);
#endif
...
NSString* BundledDYLIBPath=[NSString stringWithFormat:@"%@/Reveal2Server.dylib",[NSBundle mainBundle].bundlePath];
